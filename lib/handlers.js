...
handlers._users.post = (data, callback) => {
  ...
  lastName =
    typeof lastName == 'string' && lastName.trim().length > 0
      ...
      : null;
  phone =
    typeof phone == 'string' && phone.trim().length == 11 ? phone.trim() : null;
  password =
...
handlers._users.get = (data, callback) => {
  ...
  const phone = typeof (
    data.queryStringObject.phone == 'string' &&
...
handlers._users.put = (data, callback) => {
  ...
  phone =
    typeof phone == 'string' && phone.trim().length == 10 ? phone.trim() : null;
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionError) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of user's checks. All checks may not have been deleted",
                          });
                        }
                      }
                    });
    ...
    _data.read('users', phone, (err, userData) => {
      ...
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          // Store the token
          _performance.mark('beginning storing data');
          _data.create('tokens', tokenId, tokenObject, (err) => {
            ...
            obs.observe({ entryTypes: ['measure', 'mark'], buffered: false });
...
handlers._tokens.get = (data, callback) => {
  ...
  const id = typeof (
    data.queryStringObject.id == 'string' &&
...
handlers._checks.post = (data, callback) => {
  ...
  protocol =
    typeof protocol == 'string' && ['http', 'https'].includes(protocol.trim())
      ? protocol.trim()
      : null;
  ...
  method =
    ...
    ['post', 'get', 'update', 'delete'].includes(method.trim())
      ? method.trim()
      : null;
  successCodes =
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              dns.resolve(hostName, (err, records) => {
                if (!err && records) {
                  ...
                  callback(400, {
                    Error:
                      'The hostname of the URL entered did not resolve to any DNS enteries',
                  });
                }
              });
            } else {
